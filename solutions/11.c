...
int main(void)
{
  const uint8_t row_count = 20;
  const uint8_t column_count = 20;
  const uint8_t matrix[row_count][column_count] = {
  ...
  for (uint8_t row = 0; row < row_count; row++)
  {
    for (uint8_t column = 0; column < column_count; column++)
    {
      ...
      // Check boundaries for each compass point in turn
      ...
      // North East
      if (row - 3 >= 0 && column + 3 < column_count)
      {
      ...
      // South East
      if (row + 3 < row_count && column + 3 < column_count)
      {
        product = matrix[row][column] * matrix[row + 1][column + 1] * matrix[row + 2][column + 2] * matrix[row + 3][column + 3];
        max_product = fmax(max_product, product);
      }
  ...
  if (max_product)
  {
    printf("Max product: %" PRIu64 "\n", max_product);
    return EXIT_SUCCESS;
  }
  else
  {
    fprintf(stderr, "No maximum product found\n");
    return EXIT_FAILURE;
  }
}
